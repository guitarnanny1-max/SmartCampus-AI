...
# Completely remove any existing SmartThemeHub definition
schema = re.sub(r'model SmartThemeHub\s*\{[^}]*\}', '', schema)

# Define the full model with all required fields
model_def = """
model SmartThemeHub {
  ...
  themeName          String?  @default("Default Theme")
  mode               String?  @default("LIGHT")
  primaryColor       String?  @default("#3b82f6")
  accentColor        String?  @default("#8b5cf6")
  fontFamily         String?  @default("Inter")
  layoutStyle        String?  @default("Modern Minimalist")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartThemeHubs" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartThemeHubs SmartThemeHub[]" + parts[1]
