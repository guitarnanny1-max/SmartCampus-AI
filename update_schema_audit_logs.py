...
# Completely remove existing definitions if any
schema = re.sub(r'model AuditLog\s*\{[^}]*\}', '', schema)
schema = re.sub(r'model WebhookLog\s*\{[^}]*\}', '', schema)

# Define models with schoolId, tenantId, and proper relations
models_def = """
model AuditLog {
...
model WebhookLog {
  ...
  schoolId  String?
  school    School?  @relation(fields: [schoolId], references: [id], onDelete: Cascade)
  event     String?  @default("webhook.event")
  payload   String?
...
schema = add_relation_to_model(schema, "School", "auditLogs AuditLog[]")
schema = add_relation_to_model(schema, "School", "webhookLogs WebhookLog[]")
schema = add_relation_to_model(schema, "Tenant", "auditLogs AuditLog[]")
schema = add_relation_to_model(schema, "Tenant", "webhookLogs WebhookLog[]")
