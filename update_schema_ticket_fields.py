...
# Completely remove any existing Ticket definition
schema = re.sub(r'model Ticket\s*\{[^}]*\}', '', schema)

# Define the full model with all required ticket fields including category and submittedBy
model_def = """
model Ticket {
  ...
  submittedBy String?  @default("Facility Operator")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "tickets" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  tickets Ticket[]" + parts[1]
