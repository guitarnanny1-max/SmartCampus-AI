...
export default async function LeadsAnalyticsPage() {
  ...
  const hotLeads = leads.filter((l: any) => l.temperature.includes("Hot")).length;
  const warmLeads = leads.filter((l: any) => l.temperature.includes("Warm")).length;
  const coldLeads = leads.filter((l: any) => l.temperature.includes("Cold")).length;

  const totalStudentsReach = leads.reduce((acc: any, l: any) => acc + (l.studentStrength || 0), 0);
  const avgScore = totalLeads > 0 ? Math.round(leads.reduce((acc: any, l: any) => acc + l.score, 0) / totalLeads) : 0;

  const stages = {
    NEW: leads.filter((l: any) => l.status === "NEW" || !l.status).length,
    CONTACTED: leads.filter((l: any) => l.status === "CONTACTED").length,
    DEMO_BOOKED: leads.filter((l: any) => l.status === "DEMO_BOOKED").length,
