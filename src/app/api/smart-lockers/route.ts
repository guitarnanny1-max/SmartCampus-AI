...
export async function GET() {
  try {
    ...
    let lockers = await prisma.smartLockerDelivery.findMany({
    ...
    if (lockers.length === 0) {
      ...
      for (const l of defaultLockers) {
        await prisma.smartLockerDelivery.create({
          data: { schoolId: school.id, ...l },
        });
      }

      lockers = await prisma.smartLockerDelivery.findMany({
        where: { schoolId: school.id },
        orderBy: { createdAt: 'desc' },
      });
    }
...
export async function POST(req: Request) {
  try {
    ...
    const locker = await prisma.smartLockerDelivery.create({
      data: {
        schoolId: school.id,
        lockerBank,
