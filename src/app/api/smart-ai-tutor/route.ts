...
export async function GET() {
  try {
    ...
    let records = await prisma.smartAiTutorProctorHub.findMany({
    ...
    if (records.length === 0) {
      ...
      for (const r of defaultRecords) {
        await prisma.smartAiTutorProctorHub.create({
          data: { schoolId: school.id, ...r },
        });
      }

      records = await prisma.smartAiTutorProctorHub.findMany({
        where: { schoolId: school.id },
        orderBy: { createdAt: 'desc' },
      });
    }
...
export async function POST(req: Request) {
  try {
    ...
    const record = await prisma.smartAiTutorProctorHub.create({
      data: {
        schoolId: school.id,
        studentName,
