...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    let records = await (prisma as any).smartExamHub.findMany({
    ...
    if (records.length === 0) {
      ...
      for (const r of defaultRecords) {
        await (prisma as any).smartExamHub.create({
          data: { schoolId: school.id, ...r },
        });
      }

      records = await (prisma as any).smartExamHub.findMany({
        where: { schoolId: school.id },
        orderBy: { createdAt: 'desc' },
      });
    }
...
export async function POST(req: any): Promise<NextResponse> {
  try {
    ...
    const record = await (prisma as any).smartExamHub.create({
      data: {
        schoolId: school.id,
        examTitle,
        courseName,
