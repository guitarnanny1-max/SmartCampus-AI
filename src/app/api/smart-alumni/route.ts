...
export async function GET() {
  try {
    ...
    let records = await prisma.smartAlumniHub.findMany({
    ...
    if (records.length === 0) {
      ...
      for (const r of defaultRecords) {
        await prisma.smartAlumniHub.create({
          data: { schoolId: school.id, ...r },
        });
      }

      records = await prisma.smartAlumniHub.findMany({
        where: { schoolId: school.id },
        orderBy: { createdAt: 'desc' },
      });
    }
...
export async function POST(req: Request) {
  try {
    ...
    const record = await prisma.smartAlumniHub.create({
      data: {
        schoolId: school.id,
        alumniName,
