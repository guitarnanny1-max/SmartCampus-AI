...
# Completely remove any existing TransportRoute definition
schema = re.sub(r'model TransportRoute\s*\{[^}]*\}', '', schema)

# Define the full model with all required transport route fields including busNo and busNumber
model_def = """
model TransportRoute {
  ...
  routeName   String?  @default("Route 1")
  busNumber   String?  @default("BUS-01")
  busNo       String?  @default("BUS-01")
  driverName  String?  @default("Driver Name")
  driverPhone String?  @default("+91 99999 99999")
  capacity    Int?     @default(40)
  occupancy   Int?     @default(30)
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "transportRoutes" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  transportRoutes TransportRoute[]" + parts[1]
