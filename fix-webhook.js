...
if (schoolMatch) {
  ...
  if (!schoolBody.includes("webhookLogs")) {
    const lines = schoolBody.split("\n");
    const filteredLines = lines.filter(line => line.trim() !== "}");
    filteredLines.push("  webhookLogs WebhookLog[]");
    schema = schema.replace(schoolMatch[0], `model School {\n${filteredLines.join("\n")}\n}`);
  }
}

const modelRegex = /model WebhookLog \{[^}]+\}/;
const newModel = `model WebhookLog {
...
console.log("✅ Added WebhookLog model and relation successfully.");
