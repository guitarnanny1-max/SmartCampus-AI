...
# Completely remove any existing Facility definition
schema = re.sub(r'model Facility\s*\{[^}]*\}', '', schema)

# Define the full model with all required facility fields including zoneName, solar, hvac, status
model_def = """
model Facility {
  ...
  name         String?  @default("Facility Name")
  zoneName     String?
  solar        String?  @default("40 kW")
  hvac         String?  @default("Optimized (22°C)")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "facilities" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  facilities Facility[]" + parts[1]
