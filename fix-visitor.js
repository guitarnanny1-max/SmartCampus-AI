...
if (schoolMatch) {
  ...
  if (!schoolBody.includes("visitorLogs")) {
    const lines = schoolBody.split("\n");
    const filteredLines = lines.filter(line => line.trim() !== "}");
    filteredLines.push("  visitorLogs VisitorLog[]");
    schema = schema.replace(schoolMatch[0], `model School {\n${filteredLines.join("\n")}\n}`);
  }
}

const modelRegex = /model VisitorLog \{[^}]+\}/;
const newModel = `model VisitorLog {
...
console.log("✅ Added VisitorLog model and relation successfully.");
