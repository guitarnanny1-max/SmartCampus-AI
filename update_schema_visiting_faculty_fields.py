...
# Completely remove any existing SmartVisitingFacultyHub definition
schema = re.sub(r'model SmartVisitingFacultyHub\s*\{[^}]*\}', '', schema)

# Define the full model with all required visiting faculty fields
model_def = """
model SmartVisitingFacultyHub {
  ...
  facultyName        String?
  department         String?
  expertise          String?  @default("Advanced Computing")
  homeInstitution    String?  @default("Global Research University")
  schedulePeriod     String?  @default("Fall Semester 2026")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartVisitingFacultyHubs" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartVisitingFacultyHubs SmartVisitingFacultyHub[]" + parts[1]
