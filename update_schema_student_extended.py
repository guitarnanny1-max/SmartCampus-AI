...
# Completely remove any existing Student definition
schema = re.sub(r'model Student\s*\{[^}]*\}', '', schema)

# Define the full model with all required student fields including admissionNumber, grade, guardianName
model_def = """
model Student {
  ...
  name            String?  @default("Student Name")
  rollNo          String?
  admissionNumber String?
  grade           String?  @default("Grade 10")
  guardianName    String?  @default("Parent / Guardian")
  email           String?
  phone           String?
  cgpa            Float?   @default(9.0)
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "students" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  students Student[]" + parts[1]
