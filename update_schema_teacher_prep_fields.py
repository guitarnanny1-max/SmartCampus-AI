...
# Completely remove any existing SmartTeacherPrepHub definition
schema = re.sub(r'model SmartTeacherPrepHub\s*\{[^}]*\}', '', schema)
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartTeacherPrepHubs" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartTeacherPrepHubs SmartTeacherPrepHub[]" + parts[1]
