...
# Completely remove any existing TimetableSlot definition
schema = re.sub(r'model TimetableSlot\s*\{[^}]*\}', '', schema)

# Define the full model with timetable slot fields
model_def = """
model TimetableSlot {
  ...
  endTime     String?  @default("10:00 AM")
  subject     String?  @default("Mathematics")
  teacher     String?  @default("Dr. Smith")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "timetableSlots" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  timetableSlots TimetableSlot[]" + parts[1]
