...
export default function ClassesPage() {
  ...
  // Persisted database state and unsaved checkbox draft are kept separate.
  const [draftSectionSubjects, setDraftSectionSubjects] = useState<
    Record<string, SectionSubjectRecord[]>
  >({});
  ...
  async function loadSectionSubjects(sectionRecords: SectionRecord[]) {
    ...
    try {
      ...
      const loadedSectionSubjects =
        Object.fromEntries(results) as Record<
          string,
          SectionSubjectRecord[]
        >;

      // Merge the freshly loaded sections into the existing state.
      // Loading/saving one section must never erase another section.
      setSectionSubjects((current) => ({
        ...current,
        ...loadedSectionSubjects,
      }));

      // Keep each section's draft independent as well.
      setDraftSectionSubjects((current) => ({
        ...current,
        ...Object.fromEntries(
          Object.entries(loadedSectionSubjects).map(
  ...
  async function saveSectionSubjects(
    ...
    try {
      ...
      // Always read the latest database state before saving.
      // This avoids stale React state causing duplicate POSTs.
      const currentResponse = await fetch(
        `/api/section-subjects?section_id=${encodeURIComponent(sectionId)}`,
        {
          credentials: "include",
          cache: "no-store",
        },
      ...
      if (!currentResponse.ok) {
        throw new Error(
          currentData?.error ||
            "Unable to load current section subject assignments.",
        );
      }

      const currentAssignments =
        (currentData?.sectionSubjects ?? []) as SectionSubjectRecord[];
      ...
      // Activate selected subjects. Reuse existing INACTIVE rows
      ...
      // Deactivate every currently ACTIVE database assignment
      ...
      // Reload the saved database state. This also resets the draft
      // state to exactly what the database contains.
                                        ...
                                        {subjects
                                          ...
                                          .map((subject) => {
                                            ...
                                            const draftAssignments =
                                              draftSectionSubjects[section.id] ?? [];
                                            ...
                                            return (
                                              ...
                                              >
                                                      ...
                                                      (current) => {
                                                        ...
                                                        const nextAssignment =
                                                          existingDraft
                                                            ...
                                                            : {
                                                                id: `draft-${section.id}-${subject.id}`,
                                                                class_id:
                                                                  classRecord.id,
                                                                section_id:
                                                                  section.id,
                                                ...
                                                {savedActive ? (
                                                  ...
                                                  <span className="shrink-0 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs font-medium text-slate-400">
                                                    Available
                                                  </span>
                                                )}
                                              </div>
