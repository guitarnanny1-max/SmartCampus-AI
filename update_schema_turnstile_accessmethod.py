...
# Completely remove any existing SmartTurnstileAccess definition
schema = re.sub(r'model SmartTurnstileAccess\s*\{[^}]*\}', '', schema)

# Define the full model with all required turnstile access fields including accessMethod
model_def = """
model SmartTurnstileAccess {
  id           String   @id @default(uuid())
  tenantId     String?
  tenant       Tenant?  @relation(fields: [tenantId], references: [id], onDelete: Cascade)
  schoolId     String?
  school       School?  @relation(fields: [schoolId], references: [id], onDelete: Cascade)
  personName   String?  @default("Visitor / Student")
  visitorName  String?  @default("Visitor Name")
  hostName     String?  @default("Host Staff")
  turnstileId  String?  @default("Gate A")
  gateName     String?  @default("Main Gate")
  gateCode     String?  @default("GATE-01")
  locationName String?  @default("Main Perimeter Gate")
  accessType   String?  @default("ENTRY")
  accessMethod String?  @default("QR_BADGE")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartTurnstileAccesses" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartTurnstileAccesses SmartTurnstileAccess[]" + parts[1]
