...
# Completely remove any existing SmartSubscriptionHub definition
schema = re.sub(r'model SmartSubscriptionHub\s*\{[^}]*\}', '', schema)

# Define the full model with all required fields including price and renewalDate
model_def = """
model SmartSubscriptionHub {
  ...
  price              String?  @default("$1,499/yr")
  renewalDate        String?  @default("August 16, 2027")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartSubscriptionHubs" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartSubscriptionHubs SmartSubscriptionHub[]" + parts[1]
