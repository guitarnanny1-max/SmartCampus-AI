...
# Completely remove any existing SmartTransportationHub definition
schema = re.sub(r'model SmartTransportationHub\s*\{[^}]*\}', '', schema)

# Define the full model with passengerOccupancy and aiTrafficRoutingMode included
model_def = """
model SmartTransportationHub {
  ...
  podVelocityKph                 Float?   @default(100.0)
  magneticSuspensionStabilityPct Float?   @default(99.0)
  passengerOccupancy             Int?     @default(0)
  aiTrafficRoutingMode           String?  @default("DYNAMIC_PREDICTIVE_FLOW_OPTIMIZATION")
...
for model_name in ["School", "Tenant"]:
    ...
    if marker in schema:
        ...
        if "smartTransportationHubs" not in sub_content:
            parts = schema.split(marker)
            schema = parts[0] + marker + "\n  smartTransportationHubs SmartTransportationHub[]" + parts[1]
